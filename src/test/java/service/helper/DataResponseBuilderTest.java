...
public class DataResponseBuilderTest {

    DataResponseBuilder dataResponseBuilder = new DataResponseBuilder();

    @Test
    public void buildDataResponseTest(){
        DataResponse response = dataResponseBuilder.buildDataResponse(getIpInfoDto(), getCountryInfo(), getExchangeRate(false));
        Assert.assertNotNull(response);
        Assert.assertTrue(response.getCurrency().contains("(1 EUR ="));
    }

    @Test
    public void buildDataResponseTestError(){
        DataResponse response = dataResponseBuilder.buildDataResponse(getIpInfoDto(), getCountryInfo(), getExchangeRate(true));
        Assert.assertTrue(response.getCurrency().contains("(Cotización en Euros no disponible en el momento)"));
    }

    private IpInfoDto getIpInfoDto(){
